...
def parse_PRIK(chunk, encryption_key):
    ...
    # Handle both old hex format and newer base64 format
    if b'|' in payload:
        # New format: base64_iv|base64_ciphertext
        iv_b64, ct_b64 = payload.split(b'|', 1)
        iv = b64decode(iv_b64)
        ...
        ciphertext = decoded
    decrypted = decode_aes256('cbc', iv, ciphertext, encryption_key)

    hex_key = re.match(
        br'^LastPassPrivateKey<(?P<hex_key>.*)>LastPassPrivateKey$',
        decrypted
    ).group('hex_key')
